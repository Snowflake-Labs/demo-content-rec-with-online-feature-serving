...
```bash
cd hybrid-table-feature-store/backend
...
```bash
cd hybrid-table-feature-store/frontend
...
hybrid-table-feature-store/snowflake/01_create_database.sql
hybrid-table-feature-store/snowflake/02_create_feature_store.sql
hybrid-table-feature-store/snowflake/03_create_entities.sql
hybrid-table-feature-store/snowflake/04_create_feature_views.sql
hybrid-table-feature-store/snowflake/05_sample_data.sql
```
...
Update `hybrid-table-feature-store/backend/.env`:
...
```
├── hybrid-table-feature-store/        # Demo 1: Hybrid Table Online Feature Store
│   ├── backend/                       #   FastAPI + Two-Tower model
│   │   ├── app/
│   │   │   ├── main.py               #   FastAPI application
│   │   │   ├── config.py             #   Settings management
│   │   │   ├── models/schemas.py     #   Pydantic models
│   │   │   ├── routers/              #   clicks, recommend, features, products
│   │   │   └── services/             #   embeddings, snowflake, recommender
│   │   ├── pyproject.toml            #   Python dependencies (uv)
│   │   └── env.example
│   ├── frontend/                      #   React + Vite + TypeScript
│   │   ├── src/
│   │   │   ├── components/           #   ProductCard, RecommendSection, etc.
│   │   │   ├── hooks/                #   useRecommendation
│   │   │   └── api/                  #   client, types
│   │   └── package.json
│   ├── scripts/
│   │   ├── setup_feature_store.py    #   Register FeatureView + enable Online Serving
│   │   └── toggle_online_serving.py  #   Toggle online serving on/off
│   └── snowflake/
...
│       └── 05_sample_data.sql
│
└── online-feature-store-integration/  # Demo 2: Postgres Online Store + REST API Inference
    ├── model/
    ...
    │   ├── 03_deploy_with_feature_integration.py  # create_service + feature_sources_per_function
    │   └── 04_test_inference.py                # Call endpoint with only USER_ID
    ├── snowflake-app/                 #   Next.js Snowflake App (UI for the demo)
    │   ├── app/                       #   Next.js App Router pages + API routes
    │   ├── components/                #   UI components (shadcn/ui)
    │   ├── lib/snowflake.ts           #   Snowflake Node.js SDK query helper
    │   ├── snowflake.yml              #   Snowflake App Runtime config
    │   └── app.yml                    #   Runtime install/run commands
    └── README.md
```
